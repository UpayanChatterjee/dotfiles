...
Singleton {
    ...
    function serializeSession(): var {
        return {
            ...
            */
            // ####################################################################
            // # ADDED: Robust session serialization with safety checks           #
            // ####################################################################
            commands: {
                logout: session.commands ? session.commands.logout : ["loginctl", "terminate-user", ""],
                shutdown: session.commands ? session.commands.shutdown : ["systemctl", "poweroff"],
                hibernate: session.commands ? session.commands.hibernate : ["systemctl", "hibernate"],
                reboot: session.commands ? session.commands.reboot : ["systemctl", "reboot"]
            },
            sizes: {
                button: session.sizes ? session.sizes.button : 80
            }
            // ####################################################################
        };
    }
